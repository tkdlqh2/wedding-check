...
- `apps/web/drizzle/0020_variable-cases.sql` (NEW — main 병합 중 0017→0020로 재배치, Story 5.8이 0017~0019 선점)
- `apps/web/drizzle/meta/0020_snapshot.json` (NEW — 0019_snapshot.json 기반으로 재구성)
- `apps/web/drizzle/meta/_journal.json` (MODIFY) — 0020 엔트리 추가
...
- 2026-07-27: 코덱스 리뷰 4라운드(3라운드 수정분만 재검토) — 낮은 심각도 2건: (1) `output_dimension: 1024`(요청)와 `EXPECTED_DIMENSIONS`(응답 검증)가 서로 다른 리터럴로 중복돼 있어 하나만 바뀌면 조용히 어긋날 수 있었음 — 요청도 같은 상수를 쓰도록 통일. (2) index 범위(0..N-1) 검사 자체를 단독으로 통과시키는 테스트(중복 없이 범위만 벗어난 경우)가 없었음 — 추가. vitest 218건 재확인, tsc/lint/build 클린. 4라운드에 걸쳐 실결함 18건을 발견·수정한 뒤 클린 확인.
- 2026-07-27: main 병합 — PR 머지 준비 중 Story 5.8(FR-18 다중 배정 등, PR #21 + 후속 fix/prototype-design-alignment PR #24)이 먼저 main에 병합돼 마이그레이션 0017~0019(ceremony_assigned_operator/instance-items-ad-hoc-group/ceremony_assignees)를 선점한 것을 발견(3.1 완료 노트가 예고한 충돌 — 나중에 머지되는 쪽이 재배치). 이 스토리의 `0017_variable-cases`를 `0020_variable-cases`로 재배치하고, `0019_snapshot.json` 기반으로 `0020_snapshot.json`을 재구성, `drizzle-kit check`로 체인 정합성 재확인. 5.8이 오퍼레이터 실행 화면을 prototype RunScreen.js 기준으로 전면 재작성해 `checklist-instance-view.css`가 충돌했으나 `.step-feedback__*`(3.1/3.2)와 `.run-*`(5.8) 클래스명이 서로 겹치지 않아 순수 텍스트 충돌만 해결(둘 다 보존). `checklist-instance-view.tsx`의 `<StepFeedback>` 통합 지점은 자동 병합되어 그대로 유지됨을 확인. schema.ts는 import문(vector/primaryKey)만 충돌, 테이블 정의는 서로 다른 위치라 자동 병합. 공유 dev/test DB 둘 다 이미 병행 세션이 적용한 0017~0019와 내가 이미 적용해둔 variable_cases가 공존하고 있어 추가 마이그레이션 적용 불필요(확인만 함). 병합 후 vitest 264건(5.8의 신규 테스트 포함) 전체 통과, tsc/lint/build 클린. Status → review.
