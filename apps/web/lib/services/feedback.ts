...
export async function saveDraftFeedback(
  ...
  if (existing.status === "confirmed") {
    // 이 스토리 범위에서는 도달 불가능하다(confirmed로 바꾸는 코드가 아직 없음, Story
    // 3.2에서 생김) — 그래도 스키마가 이미 status를 지원하므로 지금 막아둔다. 나중에
    // 3.2가 확정 기능을 추가했을 때 이 화면(피드백 재입력 패널)을 통해 확정된 피드백이
    // 조용히 덮어써지는 안전 결함을 원천 차단하기 위한 방어적 코딩(AD-8).
    throw new FeedbackValidationError("이미 확정된 피드백은 수정할 수 없습니다");
  }
  return feedbackRepo.updateContent(existing.id, trimmed);
}
